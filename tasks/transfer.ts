...
task("transfer", "do transfer")
    ...
    .setAction(async (taskArgs, hre) => {
        ...
        const transferTx: ContractTransaction = await CryptonToken.transfer(taskArgs.to, taskArgs.value);

        const receipt: ContractReceipt = await transferTx.wait();

        const TransferEvent = receipt.events? receipt.events[0]: {args: {_from: "", _to: "", _value: ""}};

        console.log("successfully transfered from ", TransferEvent.args?._from, "to",  TransferEvent.args?._to, TransferEvent.args?._value.toString(), "of", "CRT");
